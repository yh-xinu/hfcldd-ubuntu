 ...
 */
/*
 * $Id: hfcl_ioctl_fx.h,v 1.1.2.6.2.1.2.4.2.2 2015/03/29 09:53:31 toyo Exp $
 */

#ifndef _H_HFCL_IOCTL_FX
#define _H_HFCL_IOCTL_FX
...
extern int hfc_fx_open(  struct inode *inode, struct file *file );
extern int hfc_fx_close( struct inode *inode, struct file *file );
#endif

#ifdef CONFIG_COMPAT
extern int hfc_fx_ioctl( struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg, int ioctl32 ); /* FCLNX-GPL-234 */
#else
extern int hfc_fx_ioctl( struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg ); 
#endif
extern int hfc_fx_inquiry(   struct port_info *pp, void *arg );
extern int hfc_fx_sciocmd(   struct port_info *pp, void *arg, int internal, int tiomeout ); /* FCLNX-0223 */
extern int hfc_fx_get_all_port_ids( struct port_info *pp, void *arg );
extern int hfc_fx_get_sid_from_wwpn( struct port_info *pp, void *arg );
extern int hfc_fx_get_wwpn_from_sid( struct port_info *pp, void *arg );
extern int hfc_fx_payload(   struct port_info *pp, void *arg );
extern int hfc_fx_get_rnid(  struct port_info *pp, void *arg );
extern int hfc_fx_set_rnid(  struct port_info *pp, void *arg );
extern int hfc_fx_adap_stat( struct port_info *pp, void *arg );
extern int hfc_fx_adap_stat_new( struct port_info *pp, void *arg );	/* FCLNX-GPL-261 */
extern int hfc_fx_hba_api(   struct port_info *pp, void *arg );
extern int hfc_fx_port_attr( struct port_info *pp, struct hfc_ioctl_api *ppi );
extern int hfc_fx_adap_attr( struct port_info *pp, struct hfc_ioctl_api *ppi ) ;
extern int hfc_fx_support(   struct port_info *pp, void *arg );
extern int hfc_fx_target_mapping( struct port_info *pp, void *arg );
extern int hfc_fx_fcp_binding(struct port_info *pp, void *arg );
extern int hfc_fx_procinfo(   struct port_info *pp, void *arg );
extern int hfc_fx_fc4stat(   struct port_info *pp, void *arg );					/* FCLNX-0404 */
extern int hfc_fx_wwn_info(   struct port_info *pp, void *arg );   //FCLNX-0178
extern int hfc_fx_mp_target_map(struct port_info *pp, void *arg );

extern int hfc_fx_adapter_enable(struct port_info *pp, void *arg );				/* hotplug */
extern int hfc_fx_adapter_disable(struct port_info *pp, void *arg );				/* hotplug */
extern int hfc_fx_scsi_scan( struct port_info *pp, void *arg );					/* hotplug */
extern int hfc_fx_read_apparam( struct port_info *pp, void *arg );        //FCLNX-0488
extern void hfc_fx_ioctl_iodone(struct scsi_cmnd *cmnd);
extern int hfc_fx_isolate_operation( struct port_info *pp, void *arg );			/* FCLNX-GPL-147 */
extern void hfc_fx_start_isolate(struct port_info *pp);
...
extern void hfc_fx_change_dev_loss_tmo(struct port_info *pp);
extern void hfc_fx_chgange_rport_dev_loss_tmo(struct port_info *pp, struct region_info *rp);
#endif
...
#define HFC_TRC_IOCTL_SC_GPNID				0x28
...
#define HFC_TRC_IOCTL_CORETRC				0x5F
...
#define HFC_TRC_IOCTL_PAYLOAD				0x90
#define HFC_TRC_IOCTL_RCV_PAYLOAD			0x91
#define HFC_TRC_IOCTL_PERFORMANCE			0x92
#define HFC_TRC_IOCTL_HFCLDD_CONF_STORE		0x93
#define HFC_TRC_IOCTL_MBTRC					0x94	/* FCLNX-GPL-FX-139 */
#define HFC_TRC_IOCTL_VPORT_LIST			0x95
#define HFC_TRC_IOCTL_RSV_PKT				0x96
#define HFC_TRC_IOCTL_PM_PKT				0x97
